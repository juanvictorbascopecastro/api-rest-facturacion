...
class Invoice extends \yii\db\ActiveRecord {
    ...
    public function getSWDL() {
        ...
        if ($wsdlSiat->success()) {
            $connection = $wsdlSiat->$connection;
...
//         $pdf->Image($fileQR,10,10,-150);
    ...
    public function contingencia($codigoMotivoEvento = 2, $idcontingencia = -1) {
        ...
        $criteria->addCondition('t."cufdEvento"=' . "'$this->cufd'");
        $criteria->addCondition('t."execute"=false');
        $criteria->addCondition('t."codigoRecepcion" is  null');

        if ($this->cafc != null) {

            if ($idcontingencia != -1)
                $criteria->addCondition("t.id=$idcontingencia");

            $criteria->addCondition("t.cafc='$this->cafc'");

            if ($idcontingencia == -1) {
                $criteria->addCondition('t."automaticExecute"=true');
            } else {
                $criteria->addCondition('t."automaticExecute"=false');
            }
        }

        $criteria->order = 't.id desc';
        $models = Contingencia::model()->findAll($criteria);

        foreach ($models as $modelContingencia) {
            $q = "select count(*) from invoice where " . '"' . "idcontingencia" . '"' . "=$modelContingencia->id ";
            $command = Yii::app()->iooxsBranch->createCommand($q);
        ...
        $modelCuis = SiatCuis::model()->findByAttributes(array('cuis' => $this->cuis));
        ...
        if ($modelContingencia->save()) {
            $this->idcontingencia = $modelContingencia->id;
            $model = Contingencia::model()->findByPk($modelContingencia->id);
            $modelInvoice = Invoice::model()->findByPk($this->id);
...
//echo "[modelSale->numeroFactura2=$this->numeroFactura;]";
        ...
        $this->codigoExcepcion =1; $modelSale->codigoExcepcion; //Por defecto, enviar cero (0) o nulo y uno (1) cuando se autorice el registro.
        ...
        if ($success) {
            ...
            if (!$this->save()) {
             print_r($this->getErrors());
                ...
                if ($this->masivaFactura == false && $this->cafc == null ) {//&& Siat::cfg()->connection == 1
                    ...
                    if ($respons != null && $respons->RespuestaServicioFacturacion->transaccion == false) {
                        $success = false;
                        $message = "<span style=\"font-size:11px\">MENSAJE DE SIAT, ERROR : [" . $respons->RespuestaServicioFacturacion->mensajesList->descripcion . "]</span>";
                    }
...
//             //RESPUESTA
                if ($respons != null && $respons->RespuestaServicioFacturacion->codigoDescripcion == 'VALIDADA') {
                    ...
                    if ($respons == null) {
                        ...
                        $success = false;
                        $message = "<span style=\"font-size:11px\">MENSAJE DE SIAT, ERROR : [" . $this->codigosRespuestas . "]</span>";
                    }
                }
...
//
    ...
    public function printLetter_01_FACTURA_COMPRA_VENTA_edu($print = 0) {
...
//Establecemos los márgenes izquierda, arriba y derecha:
        ...
        $pdf->Cell(50, 4, '', 0, 'C', 'C');
        $pdf->Cell(80, 4, $razonSocialEmisor, 0, 'C', 'C');
        //$pdf->Cell(80, 4, 'ALBASE S.R.L.', 0, 'C', 'C');
        $pdf->SetFont('Arial', 'B', 8);
        // $pdf->Cell(15, 4, '', 0, 'L');
        $pdf->Cell(31, 4, 'NIT :', 0, 'L');
        $pdf->SetFont('Arial', '', 8);
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(50, 4, '', 0, 'C', 'C');
        $municipio = '  Tel:' . $telefono . ' - ' . $municipio;
        $pdf->Cell(80, 4, $municipio, 0, 'C', 'C');
        $pdf->SetFont('Arial', 'B', 8);
        //$pdf->Cell(5, 4, '', 0, 'L');
        $pdf->Cell(31, 4, 'COD AUTORIZACION :', 0, 'L');
        ...
        $pdf->Cell(40, 4, 'FECHA:', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, System::dateFormat($fechaEmision, 'd/m/Y ') . $hora . ' ' . $horario, 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(80, 4, 'NIT/CI/CEX:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, $numeroDocumento, 0, 0, 'L');
        ...
        $pdf->SetFont('Arial', '', 7);
        $nombreRazonSocial = iconv('utf-8', 'cp1252', $nombreRazonSocial);
        $pdf->Cell(29, 4, $nombreRazonSocial, 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(80, 4, 'COD. CLIENTE:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, $this->codigoCliente, 0, 0, 'L');
        ...
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(40, 4, 'Nombre Estudiante:', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 7);
        $nombreEstudiante = 'JUAN SANCHES GUTIERREZ';
        $nombreEstudiante = iconv('utf-8', 'cp1252', $nombreEstudiante);
        $pdf->Cell(29, 4, $nombreEstudiante, 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(80, 4, 'Periodo Facturado:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, 'PERIODO 2024', 0, 0, 'L');
...
//
    ...
    public function printLetter_01_FACTURA_COMPRA_VENTA_H($print = 0) {
...
//            $linea1 = 'Especialidad: Traumatologia';
//            $linea2 = 'Reduccion de fractura';
//            $linea3 = 'Quirofano: 3';
//            $linea4 = 'Medico: CARLO ROSSO';
//            $linea5 = 'Especialidad: Traumatologia';
//            $linea6 = 'Nit: 287678578';
//            $linea7 = 'Nro. Factura: 9577';


            $linea1 = 'Especialidad: oftalmologia';
            $linea2 = '';
            $linea3 = '';
            $linea4 = 'Medico: DANIEL BUSTAMATE';
            $linea5 = 'Especialidad: oftalmologia';
            $linea6 = 'Nit: 954457785';
            $linea7 = 'Nro. Factura: 589';

            $pdf->SetFont('Arial', '', 9);
            $pdf->Cell($w[0], 4, '', 'L', 0, 'C');
            $pdf->Cell($w[1], 4, '', 'L', 0, 'C');
            $pdf->Cell($w[2], 4, '', 'L', 0, 'C');
            $pdf->Cell($w[3], 4, $linea1, 'L', 0, 'L');
            $pdf->Cell($w[4], 4, '', 'L', 0, 'C');
            $pdf->Cell($w[5], 4, '', 'L', 0, 'C');

            $pdf->Cell($w[6], 4, '', 'LR', 0, 'C');
            $pdf->Ln();
...
//
    ...
    public function printLetter_01_FACTURA_COMPRA_VENTA_abase($print = 0) {
...
//Establecemos los márgenes izquierda, arriba y derecha:
        ...
        $pdf->Ln();
        if ($model->idtypeCharge == 1) { //1 CONTADO
            $pdf->Cell(69, 4, '', 0, 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(80, 4, 'FORMA DE PAGO:', 0, 0, 'R');
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(29, 4, 'CONTADO', 0, 0, 'L');
            $pdf->Ln();
        }

        if ($model->idtypeCharge == 2) { //2 CREDIT
            $fecha_actual = $model->dateCreate;
//sumo 1 día
            $date = date("d-m-Y", strtotime($fecha_actual . "+ 59 days"));

            $pdf->Cell(69, 4, '', 0, 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(80, 4, 'FORMA DE PAGO:', 0, 0, 'R');
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(29, 4, "CREDITO (Vence el $date)", 0, 0, 'L');
            $pdf->Ln();
        }
...
//
    ...
    public function printRoll_02_FACTURA_DE_ALQUILER_DE_BIENES_INMUEBLES($print = 0) {
...
//$municipio = 'LA PAZ - BOLIVIA';
        ...
        $pdf = new FPDF('P', 'mm', array(240 + sizeof($products) * 7, 79));
//Establecemos los márgenes izquierda, arriba y derecha:
        $pdf->SetMargins(6, 7, 7);
        ...
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'PERIODO FACTURADO:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(0, 4, (string) $xml->cabecera->periodoFacturado, 0);
...
// END RQ
    ...
    public function printLetter_02_FACTURA_DE_ALQUILER_DE_BIENES_INMUEBLES($print = 0) {
...
//Establecemos los márgenes izquierda, arriba y derecha:
        ...
        $pdf->AddPage();
        if (file_exists($fileLogo)) {
            $pdf->Image($fileLogo, 20, 5);
            $pdf->Cell(80, 20, '', 0, 'C', 'C');
            $pdf->Ln();
        }
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(40, 4, '', 0, 'L');
        $pdf->Cell(30, 4, 'COD AUTORIZACION', 0, 'L');
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(40, 4, $this->cuf, 0, 'L');
        $pdf->Ln();
        ...
        $pdf->Cell(149, 4, 'Periodo Facturado:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, (string) $xml->cabecera->periodoFacturado, 0, 0, 'L');
        ...
        $pdf->Cell($w[0], 4, 'CODIGO', 'LT', 0, 'C');
        $pdf->Cell($w[1], 4, 'CANTIDAD', 'LT', 0, 'C');
        $pdf->Cell($w[2], 4, 'UNIDAD DE', 'LT', 0, 'C');
        $pdf->Cell($w[3], 4, 'DESCRIPCION', 'LT', 0, 'C');
        $pdf->Cell($w[4], 4, 'PRECIO', 'LT', 0, 'C');
        $pdf->Cell($w[5], 4, 'DESCUENTO', 'LT', 0, 'C');
        $pdf->Cell($w[6], 4, 'SUBTOTAL', 'LTR', 0, 'C');
        $pdf->Ln();

        $pdf->Cell($w[0], 4, 'PRODUCTO/', 'L', 0, 'C');
        $pdf->Cell($w[1], 4, '', 'L', 0, 'C');
        $pdf->Cell($w[2], 4, 'MEDIDA', 'L', 0, 'C');
        $pdf->Cell($w[3], 4, '', 'L', 0, 'C');
        $pdf->Cell($w[4], 4, 'UNITARIO', 'L', 0, 'C');
        $pdf->Cell($w[5], 4, '', 'L', 0, 'C');
        $pdf->Cell($w[6], 4, '', 'LR', 0, 'C');
...
// 1
        ...
        $pdf->Cell($w[2], 4, '', 'T', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal + $this->descuentoAdicional, 'number(2)');
        $pdf->Cell(120, 4, $montoTotal, 'T', 0, 'R', false);
        $pdf->Ln();
//fin
// 2
        $pdf->SetFont('Arial', '', 10);
        ...
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal, 'number(2)');
        $pdf->Cell(120, 4, SGridView::number($this->descuentoAdicional, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();
//fin
// 3
        $pdf->SetFont('Arial', '', 10);
        ...
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal, 'number(2)');
        $pdf->Cell(120, 4, $montoTotal, '', 0, 'R', false);
        $pdf->Ln();
//fin
// 4
        $pdf->SetFont('Arial', '', 10);
        ...
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoGiftCard = SGridView::number($this->montoGiftCard, 'number(2)');
        $pdf->Cell(120, 4, $montoGiftCard, '', 0, 'R', false);
        $pdf->Ln();
//fin
// 5
        $pdf->SetFont('Arial', 'B', 10);
...
// 6
        ...
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoTotalSujetoIva = SGridView::number($this->montoTotalSujetoIva, 'number(2)');
        $pdf->Cell(120, 4, $montoTotalSujetoIva, '', 0, 'R', false);
        $pdf->Ln();
        $pdf->Ln();
//fin
...
//RQ - INVOICE
        ...
        QRcode::png($codeText, $fileQR, 'L', 4, 2, true);
// $pdf->Image($fileQR,10,10,-150);
        $pdf->Cell(0, 15, $pdf->Image($fileQR, 175, $pdf->GetY(), -155), 0, 0, 'R', false);
// END RQ
        $pdf->Ln();
...
//
    ...
    public function printRoll_06_FACTURA_DE_SERVICIO_TURISTICO_Y_HOSPEDAJE($print = 0) {
...
//
//            $pdf->Ln();
            $codigoTipoHabitacion = $productDetail->codigoTipoHabitacion;
            if ($codigoTipoHabitacion != null && $codigoTipoHabitacion != '') {

                $criteria = new CDbCriteria;
                $criteria->addCondition('t."codigoClasificador"= ' . $codigoTipoHabitacion);
                $modelHabitacion = SincronizarParametricaTipoHabitacion::model()->find($criteria);

                $habitacion = $modelHabitacion->descripcion;
                $habitacion = iconv('utf-8', 'cp1252', $habitacion);
                $pdf->MultiCell(68, 4, $habitacion, 0);
            }
...
// END RQ
    ...
    public function printLetter_06_FACTURA_DE_SERVICIO_TURISTICO_Y_HOSPEDAJE($print = 0) {
...
//
    ...
    public function printRoll_11_FACTURA_SECTORES_EDUCATIVOS($print = 0) {
...
//Establecemos los márgenes izquierda, arriba y derecha:
        ...
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'NOMBRE ESTUDIANTE:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(0, 4, (string) $xml->cabecera->nombreEstudiante, 0);
...
// END RQ
    ...
    public function printLetter_11_FACTURA_SECTORES_EDUCATIVOS($print = 0) {
...
//Establecemos los márgenes izquierda, arriba y derecha:
        ...
        $pdf->Cell(40, 4, 'NOMBRE ESTUDIANTE:', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 7);
        $nombreEstudiante = (string) $xml->cabecera->nombreEstudiante;
        $nombreEstudiante = iconv('utf-8', 'cp1252', $nombreEstudiante);
        $pdf->Cell(29, 4, $nombreEstudiante, 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(80, 4, 'PERIODO FACTURADO:', 0, 0, 'R');
...
// $pdf->Image($fileQR,10,10,-150);
        ...
        $pdf->Ln();
        $pdf->Cell(0, 20, $pdf->Image($fileQR, 10, $pdf->GetY() - 20, 22), 0, 0, 'R', false);
// END RQ
//        $pdf->Ln();
//        $pdf->SetFont('Arial', 'B', 10);
//        $text = '"EL VALOR DE LA FACTURA DEBERÁ SER PAGADO EN DÓLARES O EN BOLIVIANOS AL TIPO DE CAMBIO DE VENTA AL MOMENTO DEL PAGO"';
...
//
    ...
    public function printLetter_11_FACTURA_SECTORES_EDUCATIVOS00($print = 0) {
...
//
    ...
    public function printRoll_13_FACTURA_DE_SERVICIOS_BASICOS($print = 0) {
...
//$municipio = 'LA PAZ - BOLIVIA';
        ...
        $pdf = new FPDF('P', 'mm', array(280 + sizeof($products) * 7, 79));
//Establecemos los márgenes izquierda, arriba y derecha:
        $pdf->SetMargins(6, 7, 7);
        ...
        //--
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'PERIDODO FACTURADO:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $add = (string) $xml->cabecera->mes . '/' . (string) $xml->cabecera->gestion;
        $add = iconv('utf-8', 'cp1252', $add);
        $pdf->MultiCell(0, 4, $add, 0);

//--
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'NRO MEDIDOR:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(0, 4, (string) $xml->cabecera->numeroMedidor, 0);
//--
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'COMSUMO PERIDODO:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(0, 4, (string) $xml->cabecera->consumoPeriodo, 0);

//--
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'BENEFICIARIO LEY 1886:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(0, 4, (string) $xml->cabecera->beneficiarioLey1886, 0);

//--
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(29, 4, 'DIRECION:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 9);
        $domicilioCliente = (string) $xml->cabecera->domicilioCliente;
        $domicilioCliente = iconv('utf-8', 'cp1252', $domicilioCliente);
        $pdf->MultiCell(0, 4, $domicilioCliente, 0);
////
...
//--
        ...
        $montoDescuentoLey1886 = (string) $xml->cabecera->montoDescuentoLey1886;
        if ($montoDescuentoLey1886 == '' || $montoDescuentoLey1886 == 0) {
            $montoDescuentoLey1886 = 0;
        }
        $montoDescuentoLey1886 = SGridView::number($montoDescuentoLey1886, 'number(2)');
        $montoDescuentoTarifaDignidad = (string) $xml->cabecera->montoDescuentoTarifaDignidad;
        if ($montoDescuentoTarifaDignidad == '' || $montoDescuentoTarifaDignidad == 0) {
            $montoDescuentoTarifaDignidad = 0;
        }
        $montoDescuentoTarifaDignidad = SGridView::number($montoDescuentoTarifaDignidad, 'number(2)');
        ...
        foreach ($xml->detalle as $productDetail) {
            ...
            $name = 'Ley 1886 ' . $montoDescuentoLey1886 . ' , Tarifa Dignidad ' . $montoDescuentoTarifaDignidad;
            $name = iconv('utf-8', 'cp1252', $name);
            $pdf->MultiCell(68, 4, $name, 0);
//            $pdf->Cell($w[0], 4, $name, 0, 0, 'L', false);
//
//            $pdf->Ln();
        ...
        $pdf->Cell($w[0], 4, 'Ajustes sujetos a IVA ', 0, 0, '', false);
        $pdf->Cell($w[1], 4, '', 0, 0, 'L', false);

        $pdf->Cell($w[2], 4, '', 0, 0, 'R', false);
        $ajusteSujetoIva = (string) $xml->cabecera->ajusteSujetoIva;
        if ($ajusteSujetoIva == '')
            $ajusteSujetoIva = 0;

        $pdf->Cell($w[3], 4, SGridView::number($ajusteSujetoIva, 'number(2)'), 0, 0, 'R', false);
        $pdf->Ln();
        //------------------------------------

        $pdf->Cell($w[0], 4, 'Tasa Aseo Urbano ', 0, 0, '', false);
        $pdf->Cell($w[1], 4, '', 0, 0, 'L', false);

        $pdf->Cell($w[2], 4, '', 0, 0, 'R', false);

        $tasaAseo = (string) $xml->cabecera->tasaAseo;
        if ($tasaAseo == '')
            $tasaAseo = 0;

        $pdf->Cell($w[3], 4, SGridView::number($tasaAseo, 'number(2)'), 0, 0, 'R', false);
        $pdf->Ln();
        //------------------------------------
        //------------------------------------

        $pdf->Cell($w[0], 4, 'Tasa por alumbrado ', 0, 0, '', false);
        $pdf->Cell($w[1], 4, '', 0, 0, 'L', false);

        $pdf->Cell($w[2], 4, '', 0, 0, 'R', false);
        $tasaAlumbrado = (string) $xml->cabecera->tasaAlumbrado;
        if ($tasaAlumbrado == '')
            $tasaAlumbrado = 0;
        $pdf->Cell($w[3], 4, SGridView::number($tasaAlumbrado, 'number(2)'), 0, 0, 'R', false);
        $pdf->Ln();
        //------------------------------------
        //------------------------------------

        $pdf->Cell($w[0], 4, 'Otras Tasas', 0, 0, '', false);
        $pdf->Cell($w[1], 4, '', 0, 0, 'L', false);

        $pdf->Cell($w[2], 4, '', 0, 0, 'R', false);
        $otrasTasas = (string) $xml->cabecera->otrasTasas;
        if ($otrasTasas == '')
            $otrasTasas = 0;
        $pdf->Cell($w[3], 4, SGridView::number($otrasTasas, 'number(2)'), 0, 0, 'R', false);
        $pdf->Ln();
        //------------------------------------
        //------------------------------------

        $pdf->Cell($w[0], 4, 'Otros pagos (Pago de cuotas', 0, 0, '', false);
        $pdf->Cell($w[1], 4, '', 0, 0, 'L', false);

        $pdf->Cell($w[2], 4, '', 0, 0, 'R', false);
        $otrosPagosNoSujetoIva = (string) $xml->cabecera->otrosPagosNoSujetoIva;
        if ($otrosPagosNoSujetoIva == '')
            $otrosPagosNoSujetoIva = 0;
        $pdf->Cell($w[3], 4, SGridView::number($otrosPagosNoSujetoIva, 'number(2)'), 0, 0, 'R', false);
        $pdf->Ln();
        //------------------------------------
// 1
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell($w[0], 4, 'TOTAL Bs', 'T', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', 'T', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', 'T', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal + $this->descuentoAdicional, 'number(2)');
        $pdf->Cell($w[3], 4, $montoTotal, 'T', 0, 'R', false);
...
// 3
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell($w[0], 4, 'SUB TOTAL A PAGAR  Bs', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal, 'number(2)');
        $pdf->Cell($w[3], 4, $montoTotal, '', 0, 'R', false);
...
// 4
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell($w[0], 4, '(-)AJUSTES NO SUJETOS A IVA Bs', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);

        $ajusteNoSujetoIva = (string) $xml->cabecera->ajusteNoSujetoIva;
        if ($ajusteNoSujetoIva == '')
            $ajusteNoSujetoIva = 0;


        $pdf->Cell($w[3], 4, SGridView::number($ajusteNoSujetoIva, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();
//fin
// 5
        $pdf->SetFont('Arial', 'B', 8);
        ...
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal - $ajusteNoSujetoIva, 'number(2)');
        $pdf->Cell($w[3], 4, $montoTotal, '', 0, 'R', false);
        $pdf->Ln();
//fin
//
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell($w[0], 4, '(-) TASAS Bs', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $pdf->Cell($w[3], 4, SGridView::number($otrasTasas + $tasaAseo + $tasaAlumbrado, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();
///
        ...
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell($w[0], 4, '(-) OTROS PAGOS NO SUJETOS A IVA Bs', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);

        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell($w[3], 4, SGridView::number($otrosPagosNoSujetoIva, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();
...
// END RQ
    ...
    public function printLetter_13_FACTURA_DE_SERVICIOS_BASICOS($print = 0) {
...
//Establecemos los márgenes izquierda, arriba y derecha:
        ...
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(40, 4, 'Direccion:', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 7);

        $domicilioCliente = (string) $xml->cabecera->domicilioCliente;
        $domicilioCliente = iconv('utf-8', 'cp1252', $domicilioCliente);
        $pdf->Cell(29, 4, $domicilioCliente, 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(80, 4, 'Benefiaciario Ley:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, (string) $xml->cabecera->beneficiarioLey1886, 0, 0, 'L');

        $pdf->Ln();

        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(40, 4, 'Consumo Periodo:', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 7);

        $pdf->Cell(29, 4, (string) $xml->cabecera->consumoPeriodo, 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(80, 4, 'Periodo Facturado:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        ...
        $pdf->Cell(29, 4, $add, 0, 0, 'L');

        $pdf->Ln();

        $pdf->Cell(149, 4, 'Nro Medidor:', 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(29, 4, (string) $xml->cabecera->numeroMedidor, 0, 0, 'L');
...
//
            $pdf->SetTextColor(255, 255, 255);
            $pdf->MultiCell($w[3], $height, $descripcion);
            $x = $pdf->GetX();
            $y = $pdf->GetY();
//
            $height = ($y - $y0);
...
//$pdf->Cell($w[3], 4, $descripcion, 'LT', 0, 'L');
            $x0 = $pdf->GetX();
            $y0 = $pdf->GetY();
            $pdf->MultiCell($w[3], $height, $descripcion, 'LT');
            $pdf->SetXY($x0 + $w[3], $y0);
//            $pdf->Cell($w[0], 4, $name, 0, 0, 'L', false);
//
//            $pdf->Ln();
            ...
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell($w[0], $height, '', 'L', 0, 'C');
            $pdf->Cell($w[1], $height, '', 'L', 0, 'C');
            $pdf->Cell($w[2], $height, '', 'L', 0, 'C');

            $lineadd = 'Ley 1886 ' . $montoDescuentoLey1886 . ' , Tarifa Dignidad ' . $montoDescuentoTarifaDignidad;
            $lineadd = iconv('utf-8', 'cp1252', $lineadd);
            $pdf->Cell($w[3], $height, $lineadd, 'L', 0, '');

            $pdf->Cell($w[4], $height, '', 'L', 0, 'C');
            $pdf->Cell($w[5], $height, '', 'L', 0, 'C');
            $pdf->Cell($w[6], $height, '', 'LR', 0, 'C');
...
//
        ...
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell($w[0], $height, '', 'LTR', 0, 'C');
        $pdf->Cell($w[1], $height, '', 'LTR', 0, 'C');
        $pdf->Cell($w[2], $height, '', 'LTR', 0, 'C');

        $lineadd = 'Ajustes sujetos a IVA ';
        $lineadd = iconv('utf-8', 'cp1252', $lineadd);
        $pdf->Cell($w[3], $height, $lineadd, 'LTR', 0, '');
        ...
        $pdf->Cell($w[4], $height, '', 'LTR', 0, 'C');
        $pdf->Cell($w[5], $height, '', 'LTR', 0, 'C');

        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell($w[6], $height, SGridView::number($ajusteSujetoIva, 'number(2)'), 'LTR', 0, 'R');

        $pdf->Ln();

        $detalles = (string) $xml->cabecera->detalleAjusteSujetoIva;
        $detalles = $detalles == '' ? array() : CJSON::decode($detalles);
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell($w[6], $height, SGridView::number($tasaAseo, 'number(2)'), 'LTR', 0, 'R');
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell($w[6], $height, SGridView::number($tasaAlumbrado, 'number(2)'), 'LTR', 0, 'R');
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell($w[6], $height, SGridView::number($otrasTasas, 'number(2)'), 'LTR', 0, 'R');
        ...
        $lineadd = 'Otros pagos (Pago de cuotas etc)';
        $lineadd = iconv('utf-8', 'cp1252', $lineadd);
        $pdf->Cell($w[3], $height, $lineadd, 'LTR', 0, '');
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell($w[6], $height, SGridView::number($otrosPagosNoSujetoIva, 'number(2)'), 'LTR', 0, 'R');

        $pdf->Ln();

        $detalles = (string) $xml->cabecera->detalleOtrosPagosNoSujetoIva;
        $detalles = $detalles == '' ? array() : CJSON::decode($detalles);
        foreach ($detalles as $detalle => $value) {
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell($w[0], $height, '', 'L', 0, 'C');
...
// 4
        ...
        $pdf->Cell(120, 4, SGridView::number($ajusteNoSujetoIva, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();

        $detalles = (string) $xml->cabecera->detalleAjusteNoSujetoIva;
        $detalles = $detalles == '' ? array() : CJSON::decode($detalles);
        foreach ($detalles as $detalle => $value) {
            $lineadd = $detalle;
            $lineadd = iconv('utf-8', 'cp1252', $lineadd);
            ...
            $pdf->SetFont('Arial', '', 9);
            $pdf->Cell($w[0], 4, $lineadd, '', 0, 'L', false);
            $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
            $pdf->Cell($w[2], 4, '', '', 0, 'R', false);

            $pdf->Cell(120, 4, '', '', 0, 'R', false);
            $pdf->Ln();
        }
//fin
// 5
...
// 4
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell($w[0], 4, '(-) TASAS B', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);

        $pdf->Cell(120, 4, SGridView::number($otrasTasas + $tasaAseo + $tasaAlumbrado, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();
...
// 4
        ...
        if ($otrosPagosNoSujetoIva == '')
            $otrosPagosNoSujetoIva = 0;
        $pdf->Cell(120, 4, SGridView::number($otrosPagosNoSujetoIva, 'number(2)'), '', 0, 'R', false);
        $pdf->Ln();
...
//
    ...
    public function printRoll_15_FACTURA_DE_ENTIDADES_FINANCIERAS($print = 0) {
...
// 1
        ...
        $montoTotalArrendamientoFinanciero = (string) $xml->cabecera->montoTotalArrendamientoFinanciero;
        if ($montoTotalArrendamientoFinanciero == '' || $montoTotalArrendamientoFinanciero == 0) {
        ...
        $pdf->Cell($w[1], 4, '', 'T', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', 'T', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal + $this->descuentoAdicional - $montoTotalArrendamientoFinanciero, 'number(2)');
        $pdf->Cell($w[3], 4, $montoTotal, 'T', 0, 'R', false);
        $pdf->Ln();
        //////////////
        ...
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell($w[0], 4, 'MONTO ARRENDAMIENTO FINANCIERO Bs', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);

        $pdf->SetFont('Arial', '', 8);
        $montoTotalArrendamientoFinanciero = SGridView::number($montoTotalArrendamientoFinanciero, 'number(2)');
        $pdf->Cell($w[3], 4, $montoTotalArrendamientoFinanciero, '', 0, 'R', false);
        $pdf->Ln();
//fin
// 2
        $pdf->SetFont('Arial', '', 8);
...
// END RQ
    ...
    public function printLetter_15_FACTURA_DE_ENTIDADES_FINANCIERAS($print = 0) {
...
// 2
        ...
        $montoTotalArrendamientoFinanciero = SGridView::number($montoTotalArrendamientoFinanciero, 'number(2)');
        $pdf->Cell(120, 4, $montoTotalArrendamientoFinanciero, '', 0, 'R', false);
        $pdf->Ln();
//fin
// 2
        $pdf->SetFont('Arial', '', 10);
...
//
    ...
    public function printRoll_16_FACTURA_DE_HOTELES($print = 0) {
...
// END RQ
    ...
    public function printLetter_16_FACTURA_DE_HOTELES0($print = 0) {
...
//
    ...
    public function printLetter_16_FACTURA_DE_HOTELES($print = 0) {
...
//
    ...
    public function printRoll_17_FACTURA_DE_HOSPITALES_CLINICAS($print = 0) {
...
//
            ...
            $especialidad = $productDetail->especialidad;
            $especialidad = iconv('utf-8', 'cp1252', 'Especialidad : ' . $especialidad);
            $pdf->MultiCell(68, 4, $especialidad, 0);

            $especialidadDetalle = $productDetail->especialidadDetalle;
            $especialidadDetalle = iconv('utf-8', 'cp1252', $especialidadDetalle);
            $pdf->MultiCell(68, 4, $especialidadDetalle, 0);

            $nroQuirofanoSalaOperaciones = $productDetail->nroQuirofanoSalaOperaciones;
            $nroQuirofanoSalaOperaciones = iconv('utf-8', 'cp1252', 'Quirofano: ' . $nroQuirofanoSalaOperaciones);
            $pdf->MultiCell(68, 4, $nroQuirofanoSalaOperaciones, 0);

            $especialidadMedico = $productDetail->especialidadMedico;
            $especialidadMedico = iconv('utf-8', 'cp1252', 'Medico :' . $especialidadMedico);
            $pdf->MultiCell(150, 4, $especialidadMedico, 0);

            $nombreApellidoMedico = $productDetail->nombreApellidoMedico;
            $nombreApellidoMedico = iconv('utf-8', 'cp1252', $nombreApellidoMedico);
            $pdf->MultiCell(68, 4, $nombreApellidoMedico, 0);

            $nitDocumentoMedico = $productDetail->nitDocumentoMedico;
            $nitDocumentoMedico = iconv('utf-8', 'cp1252', 'Nit:' . $nitDocumentoMedico);
            $pdf->MultiCell(68, 4, $nitDocumentoMedico, 0);

//            $nroMatriculaMedico = $productDetail->nroMatriculaMedico;
//            $nroMatriculaMedico = iconv('utf-8', 'cp1252', $nroMatriculaMedico);
//            $pdf->MultiCell(68, 4, $nroMatriculaMedico, 0);

            $nroFacturaMedico = $productDetail->nroFacturaMedico;
            $nroFacturaMedico = iconv('utf-8', 'cp1252', 'Nro factura: ' . $nroFacturaMedico);
            $pdf->MultiCell(68, 4, $nroFacturaMedico, 0);
...
// END RQ
    ...
    public function printLetter_17_FACTURA_DE_HOSPITALES_CLINICAS($print = 0) {
...
//
    ...
    public function printRoll_22_FACTURA_TELECOMUNICACIONES($print = 0) {
...
// END RQ
    ...
    public function printLetter_22_FACTURA_TELECOMUNICACIONES($print = 0) {
...
//
    ...
    public function printRoll_34_FACTURA_DE_SEGUROS($print = 0) {
...
//fin
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell($w[0], 4, '(-)AJUSTE IVA Bs', '', 0, 'L', false);
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);

        $ajusteAfectacionIva = (string) $xml->cabecera->ajusteAfectacionIva;
        if ($ajusteAfectacionIva == '' || $ajusteAfectacionIva == 0) {
            $ajusteAfectacionIva = 0;
        }

        $pdf->SetFont('Arial', '', 8);
        $ajusteAfectacionIva = SGridView::number($ajusteAfectacionIva, 'number(2)');
        $pdf->Cell($w[3], 4, $ajusteAfectacionIva, '', 0, 'R', false);
        $pdf->Ln();
// 4
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell($w[0], 4, 'MONTO GIFT CARD Bs', '', 0, 'L', false);
...
// 5
        ...
        $pdf->Cell($w[1], 4, '', '', 0, 'L', false);
        $pdf->Cell($w[2], 4, '', '', 0, 'R', false);
        $montoTotal = SGridView::number($this->montoTotal - $this->montoGiftCard - $ajusteAfectacionIva, 'number(2)');
        $pdf->Cell($w[3], 4, $montoTotal, '', 0, 'R', false);
        $pdf->Ln();
//fin
// 6
...
// END RQ
    ...
    public function printLetter_34_FACTURA_DE_SEGUROS($print = 0) {
...
// 3
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(105, 4, '', '', 0, 'L', false);
        $pdf->Cell(70, 4, '(-)AJUSTE IVA Bs', 'LTRB', 0, 'R', true);
        $ajusteAfectacionIva = (string) $xml->cabecera->ajusteAfectacionIva;
        if ($ajusteAfectacionIva == '' || $ajusteAfectacionIva == 0) {
            $ajusteAfectacionIva = 0;
        }
        $ajusteAfectacionIva = SGridView::number($ajusteAfectacionIva, 'number(2)');
        $pdf->Cell(20, 4, $ajusteAfectacionIva, 'LTRB', 0, 'R', true);
        $pdf->Ln();
//fin
// 4
        $pdf->SetFont('Arial', '', 10);
...
//
    ...
    public function printLetter_34_FACTURA_DE_SEGUROS0($print = 0) {
